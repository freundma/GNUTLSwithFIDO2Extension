...
a list of allow credentials (ID of the PKCS to use). Therefore a doubled handshake is performed to confidentially send the usernname first and
...
the 32 random bytes and the actual username. The server also calculates the SHA256-Hash over the concatenation of both a saves a tripel of
